	...
	if ok and snacks.picker then
		...
		snacks.picker.pick({
			...
			format = function(item)
				local prefix = item.type == "private" and "\xE2\x9C\x89 " or ""
				local label = prefix .. item.text
			...
			confirm = function(picker, item)
				picker:close()
				if item then
					on_select({ id = item.id, title = item.text, unread = item.unread, type = item.type })
				else
					on_select(nil)
				end
			end,
		...
		vim.ui.select(items, {
			...
			if item then
				on_select({ id = item.id, title = item.title, unread = item.unread, type = item.type })
			else
				on_select(nil)
			end
		end)
